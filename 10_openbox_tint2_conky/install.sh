...
for d in /etc/skel/  /home/*/ /root; do
	...
	dc="$d"
    # Skip dirs in /home that are not  user home
	...
	dc="$dc/.config/"; [ ! -d "$dc" ] && mkdir -v "$dc" && chown -R $(stat "$(dirname "$dc")" -c %u:%g) "$dc"
	dc="$dc/conky/";  [ ! -d "$dc" ] && mkdir -v "$dc" && chown -R $(stat "$(dirname "$dc")" -c %u:%g) "$dc"
	# Copy all conky configs
	touch "$dc/each conky.conf file here is autoloaded"
	cp -v "$base_dir/"conky.conf "$dc" && chown $(stat "$(dirname "$dc")" -c %u:%g) "$dc"/conky.conf
